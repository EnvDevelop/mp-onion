...
function canManageComplaints(req: Express.Request) {
  // Владелец платформы имеет полный доступ к любым обращениям
  if (req.user && (req.user.role === UserRole.OWNER || req.user.role === UserRole.SECURITY)) {
    return true;
  }
...
async function canManageShopComplaints(req: Express.Request, shopId: number) {
  // Владелец платформы всегда имеет права на управление жалобами магазинов
  if (req.user && (req.user.role === UserRole.OWNER || req.user.role === UserRole.SECURITY)) {
    return true;
  }
