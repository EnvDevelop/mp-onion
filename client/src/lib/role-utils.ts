...
export function isHeadAdmin(user: User | null) {
  if (!user) return false;
  return isOwner(user) || user.role === UserRole.HEADADMIN;
}
...
export function isModerator(user: User | null) {
  if (!user) return false;
  return isOwner(user) || user.role === UserRole.MODERATOR;
}
...
export function isStaff(user: User | null) {
  if (!user) return false;
  // Владелец всегда имеет права сотрудника
...
export function isShopStaff(user: User | null) {
  if (!user) return false;
  // Владелец всегда имеет права сотрудника магазина
  if (isOwner(user)) return true;
...
export function canManageShop(user: User | null) {
  if (!user) return false;
  return isOwner(user) || isAdmin(user) || user.role === UserRole.SHOP_OWNER || user.role === UserRole.SHOP_MAIN;
}
