...
export function ComplaintChat({
  ...
  const isStaff = (user: User) => {
    // Владельцы платформы имеют доступ ко всем возможностям
    if (user.role === UserRole.OWNER || user.role === UserRole.SECURITY) {
      return true;
    }
  ...
  return (
    <Card className="h-[600px] flex flex-col">
      <div className="p-4 border-b">
        ...
        {isStaff(currentUser) && complaint.status === "IN_PROGRESS" && (
          <div className="mt-4 flex gap-2">
            <Button 
              ...
              disabled={resolveComplaintMutation.isLoading}
            >
            ...
            <Button 
              ...
              disabled={rejectComplaintMutation.isLoading}
            >
        ...
        {/* Assign button - Only staff can see this, and if not assigned or already assigned to current user */}
        {isStaff(currentUser) && (complaint.status === "В ожидании" || complaint.assignedToId === currentUser.id) && (
          // Additional check: OWNER role can always assign complaints to themselves
          (currentUser.role === UserRole.OWNER || currentUser.role === UserRole.SECURITY || !complaint.assignedToId) && (
            <Button 
              variant="outline" 
              size="sm" 
              className="ml-2" 
            ...
            >
              {assignComplaintMutation.isLoading ? (
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={!rejectReason.trim() || rejectComplaintMutation.isLoading}
            >
              {rejectComplaintMutation.isLoading ? (
