...
// Функция для формирования URL чата
const getChatUrl = (chat: ChatItem): string => {
  switch (chat.type) {
    case 'shop':
      // Обновленный формат URL для чатов с магазинами
      return `/chats=${chat.id}&shop=${chat.shopId}`;
    case 'complaint':
      return `/complaints/${chat.id}`;
    case 'shopComplaint':
      return `/shop-complaints/${chat.shopId}/${chat.id}`;
    case 'clientChat':
      return `/shop-chats/${chat.id}`;
    case 'shopAppeal':
      return `/shop-appeals/${chat.id}`;
    case 'shopStaffChat':
      // Используем новый формат и для сотрудников магазинов
      return `/chats=${chat.id}&shop=${chat.shopId}`;
    default:
...
export default function ChatsPage() {
  ...
  const allChats: ChatItem[] = [
    ...(shopChats || []).map(chat => ({
      ...
      type: 'shop' as const,
      title: chat.shopName,
      url: `/chats=${chat.id}&shop=${chat.shopId}`,
    ...
    ...(complaints || []).filter(c => c.isShopComplaint).map(complaint => ({
      ...
      title: complaint.title,
      url: `/shop-complaints/${complaint.shopId}/${complaint.id}`,
      lastMessage: complaint.description || null,
      lastMessageAt: typeof complaint.createdAt === 'string' 
        ? complaint.createdAt 
        : (complaint.createdAt instanceof Date 
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center mb-4">
          <div className="relative w-full">
            <Input
              ...
              className="pl-10 bg-gray-900 border-gray-700 text-foreground placeholder:text-gray-500"
              value={searchQuery}
        ...
        <CardContent className="p-0">
          {filteredChats.length > 0 ? (
                  ...
                  <div className="flex items-center gap-4 py-3 px-4 hover:bg-gray-800 transition-colors cursor-pointer">
                    ...
                    <div className="space-y-1 flex-1">
                      <div className="flex justify-between">
                        <h3 className="font-medium leading-none text-foreground">{chat.title}</h3>
            ...
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <h3 className="text-lg font-medium text-foreground">Чаты не найдены</h3>
